...
-- This script shows which users exist and which need to sign up first
...
-- Step 2: Add admin role for existing users only
...
SELECT
    CASE
        WHEN EXISTS (SELECT 1 FROM auth.users au WHERE au.email = up.email) THEN '✅ Admin Added'
        ELSE '⏳ Needs Sign Up'
    END as status,
    up.email,
    up.role,
